...
(select MemberName,'0192023a7bbd73250516f069df18b500','2' from gym.member);
#回写用户id至会员表
update gym.member m left join gym.adminuser u on u.adminName = m.MemberName set m.adminId = u.adminId where u.permission = '2'


#将教练表信息关联至用户表
insert into gym.adminuser (adminName,adminPassword,permission)
(select coachName,'0192023a7bbd73250516f069df18b500','3' from gym.coach);
#回写用户id至教练表
update gym.coach c left join gym.adminuser u on u.adminName = c.coachName set c.adminId = u.adminId where u.permission = '3'